...
public class FileUtil {
	...
	//- 디렉터리일 경우는 다음과 같은 절차를 걸친다
	//		1.대상 위치 안에 원본 디렉터리 이름과 같은 디렉터리 생성
	// 		2.원본 디렉터리 안에 있는 파일/디렉터리를 1번에서 만든 위치로 복사
	
	public static void copy(File origin, File target) throws IOException {//origin 복사할 대상
		if(origin.isFile()) {
			//읽기 위한 스트림 생성 및 조합
			FileInputStream readStream = new FileInputStream(origin);//예외처리하면 안됨
			...
			File dir = new File(target,origin.getName());//target.origin.getNmae()
			dir.mkdirs();
			
			//origin내용물을 dir 내부에 복사 
	...
	public static void delete(File target) {
		if(target.isFile()) {//파일은 그냥 삭제
			target.delete();
		}
		else if(target.isDirectory()) {//내용물을 지우고 삭제 
			File[] list = target.listFiles();
			for(File file : list) {
				delete(file);//재귀 호출(같은 기능을 재호출)//계속 재검사를 해서 끝까지!!
			}
			target.delete();
		}
	}
