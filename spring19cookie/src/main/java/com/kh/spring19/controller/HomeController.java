	...
	/*
	 	쿠키는 Cookie 클래스의 객체로 생성
	 	 생성시 다음 정보를 설정할 수 있다
	 	  ...
	 	  (중요) 사용자가 쿠키를 가지고 있어야 하므로 
	 	  응답 객체(HttpServletResponse - 인터셉터에서 사용했었음)에 탑재시켜야 한다
	 */
	@RequestMapping("/create1")
	public String create1(HttpServletResponse response) {
		Cookie cookie = new Cookie("popup","false");
		cookie.setMaxAge(Integer.MAX_VALUE);//가장 큰 숫자
...
//			@RequestParam(required = false, defaultValue = "N") String remember) {//미체크시 N
		...
		//if(로그인 성공시){
			if(remember != null) {//아이디 저장하기를 체크했다면
				Cookie cookie = new Cookie("saveId", memberId);//쿠키생성
				cookie.setMaxAge(4*7*24*60*60);//4주(보통2~4주로 설정)
