...
public class KakaoPayController {
	...
	public String test3Purchase(@ModelAttribute PurchaseListVO listVO, Model model) {
		...
		for(PurchaseVO vo : purchaseList) {//사용자가 선택한 번호와 수량의 목록을 반복하며
			...
			total += confirmVO.getTotal();//총 구매금액 합계
		}
	...
	//[1] 결제 상세 번호로 상세 정보 객체(PaymentDetailDto)를 조회
	...
	//- 결제 대표 정보의 잔여금액을 차감해야 한다(payment)
...
//		if(paymentDetailDto.getPaymentDetailStatus().equals("취소")) {//너무 구구절절
		if(paymentDetailDto == null || paymentDetailDto.isCanceled()) {//이미 취소된 요청이라면 차단!/가상의 게터메소드 생성
		...
		KakaoPayCancelRequestVO request = KakaoPayCancelRequestVO.builder()
					...
					.cancelAmount(
							paymentDetailDto.getPaymentDetailProductPrice() //상품판매가
									* paymentDetailDto.getPaymentDetailProductQty()//구매 수량
							)
				.build();
		//[4]
		KakaoPayCancelResponseVO response = kakaoPayService.cancel(request);
		...
		paymentDao.cancel(PaymentDto.builder()
						.paymentNo(paymentDto.getPaymentNo())//결제 대표 번호 
