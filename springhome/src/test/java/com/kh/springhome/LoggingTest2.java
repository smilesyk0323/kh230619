...
//			private Logger log = LoggerFactory.getLogger(LoggingTest2.class);
		...
		public void test() {
			...
			//- 총 6가지가 존재
			//- fatal - 시스템이 다운되는 오류. 개발자가 작성 불가. (가장긴급한 메세지)
			//- error - 기능이 다운되는 오류. 
			//- warm - 기능에 문제는 없지만 성능저하나 메모리 낭비 등 위험이 예상되는 상황
			//- info - 시스템에서 발행하는 정보. 사용자에게 보여지는 정보.
			//- debug - 개발자에게 필요한 정보. 오류 해결을 위한 정보.(info가 나와야 debug도 있음,info없이 debug는 있을수 없음)
			//- trace - 시스템의 모든 작업에 대한 상세정보. 직접 작성을 피해야 함.
			log.trace("메세지1");
			log.debug("메세지2");
			log.info("메세지3");
			log.warn("메세지3");
